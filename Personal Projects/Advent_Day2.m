...
massData = dlmread("Data_Day2.txt");
fuelRequired = sum(floor(massData/3)-2);
fprintf("The total fuel required is %.0f\n", fuelRequired);

i = 1;
additionalFuel = zeros(50);
while additionalFuel(i) == 0
    if i == 1
        additionalFuel(i) = floor((fuelRequired/3))-2;
    else
        additionalFuel(i) = floor((additionalFuel(i-1)/3))-2;
    end
    if additionalFuel(i) <= 0
        additionalFuel(i) = 0;
        break; 
    end
    i=i+1;
...
newFuelRequired = fuelRequired + sum(additionalFuel, 'all');
fprintf("The new fuel required is %.0f\n", newFuelRequired);
